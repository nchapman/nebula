...
#+BEGIN_SRC bash
usage:
	cargo +nightly r --release --example text_to_speech --features tts -- <model_type>
#+END_SRC
...
#+BEGIN_SRC bash
    ...
    cd ..
#+END_SRC
After running these commands, download models from [[https://drive.google.com/file/d/15UCvTNTwSacP_e7ZjNzib4iiHSegkM8Q/view?usp=sharing][my Google Drive]] manually. Unpack the =*.pt= files into =models/torch-scripts= folder. Then run:
#+BEGIN_SRC bash
    cargo +nightly r --release --example text_to_speech --features tts -- style
#+END_SRC
*** Parler-mini
#+BEGIN_SRC bash
    cargo +nightly r --release --example text_to_speech --features tts -- parler-mini
#+END_SRC
*** Parler-large
#+BEGIN_SRC bash
    cargo +nightly r --release --example text_to_speech --features tts -- parler-large
#+END_SRC
